...
@time minCostTest2, minTsListTest2, P1ListTest2, P2ListTest2, P3ListTest2, PeListTest2 = generateAndSolve(PressedWaterOneStorageOneCompressor(), MinimizeCost(), VaryLoadVaryArea(), GoldenRatioMethod();
	...
	Tsmin = 120.0,
	# 求解参数
	dT = dT / 100,# 状态参数高温蓄热温度离散步长
	dt = dt,# 时间步长
	smoother = smoother,
)
println(minCostTest2)
...
plt2 = plot(tList .+ 0.5, [P1ListTest2 P2ListTest2 P3ListTest2 PeListTest2], label = ["P1" "P2" "P3" "Pe"], ylims = (0, 2.5),title="$minCostTest2")
plot!(plt2, tList, (minTsListTest2[1:end-1] .- 120) / 100, label = "Ts")
end
#8.341464137165978
