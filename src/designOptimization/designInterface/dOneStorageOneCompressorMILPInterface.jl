"""
    PressedWaterOneStorageOneCompressor_MILP 设计优化接口
    经济参数与双层优化总成本计算
"""

# 经济参数
struct MILPFinanceParameters <: AbstractFinanceParameters
    Plow_cost::Float64       # 低温热泵价格 (¥/kW)
    Phigh_cost::Float64      # 高温热泵价格 (¥/kW)
    Pelec_cost::Float64      # 电锅炉价格 (¥/kW)
...
function totalPresentWorth(
    ...
    financeParams::MILPFinanceParameters,
...
function get_bb_cost(
    ...
    fp::MILPFinanceParameters = MILPFinanceParameters(
    ...
    function bb_cost(x::Vector{Float64})
        ...
        if operationResult isa MILPModelResult
            obj = operationResult.objective
        else
            obj = operationResult[1]
        end

        if obj < 1.0
            @warn "目标函数值:$(obj),小于1.0，请检查参数:$(round.(x, digits=3))"
        end
