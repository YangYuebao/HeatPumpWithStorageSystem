...
function generateAndSolve(::PressedWaterDoubleStorage, ::MinimizeCost;
	...
	@variable(model, TstorageTankMax >= T3[i = 1:m] >= Tair_min, start = minTeh)# 低温蓄热温度
	...
	if isFesable in [FEASIBLE_POINT, NEARLY_FEASIBLE_POINT]
		dfResult = DataFrame(
			...
			"低温罐温度" => value.(T3),
			"高温罐温度" => value.(T8),
			"低温热泵蒸发器温度_热回收"=>fill(TWaste, 24),
			"低温热泵蒸发器温度_空气源"=>Tair,
			"低温热泵冷凝器温度"=>value.(Tc_l),
			"低温热回收热泵COP" => map(i -> COPl(Te_l1, value(Tc_l[i])), 1:m),
			"低温热回收热泵功率" => Pl1List,
			"低温空气源热泵COP" => map(i -> COPl(Te_l2[i], value(Tc_l[i])), 1:m),
			"低温空气源热泵功率" => Pl2List,
			"高温热泵蒸发器温度"=>value.(Te_h),
			"高温热泵冷凝器温度_蓄热"=>value.(Tc_h1),
			"高温热泵冷凝器温度_供热"=>value.(Tc_h2),
